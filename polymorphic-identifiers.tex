...
The identifiers built into existing programming languages are commonly bypassed by developers.
...
mechanism to the programming language itself, with enough flexibility to adapt to changing requirements.

Our starting point was the Objective-Smalltalk~\cite{objst} dialect of Smalltalk~\cite{Goldberg1983}, which has objects
that communicate via messages as well as early bound identifiers accessing variables.  We extended
this dialect with our late-bound Polymorphic Identifiers and applied this new dialect to a number of
projects, but the same principles could be applied to other languages such as Java, as depicted in
Figure~\ref{pi-java}.


\begin{figure}[htbp]
\begin{lstlisting}[style=L,label=pi-java,caption=Draw remote image and download an RFC with Polymorphic Identifiers in Java.]
	drawingContext.drawImage( flickr:3559417932_7208103ed8_m.jpg );
	file:rfc1738.txt = http://http://datatracker.ietf.org/doc/rfc1738;
\end{lstlisting}
\end{figure}
...
schemes as a user-level filesystem using FUSE~\cite{fuse} or as a web-server using libmicrohttp~\cite{libmicrohttp}.
...
\begin{figure}[htbp]
\begin{lstlisting}[style=L,label=setvalue-env,caption=Set and check value in env: scheme.]
...
of applications, ranging from an iPad language learning game via the site generator that produces the {\tt objective.st} web-site to a
live remote IDE that uses HTTP to upload code and download debug information from running processes.  Figure~\ref{pi-inuse}
has three IDE windows that show, respectively, the setup code for the web-site, part of the IDE code itself that uploads new method definitions,
and finally a window with a live connection to the language learning game running in the iOS simulator in the background.

\begin{figure}[htbp]
\centering
\includegraphics[scale=0.25,page=1]{PolymorphicIdentifiersInUse.png}
\caption{Polymorphic Identifiers in use}
\label{pi-inuse}
\end{figure}


The game had a large number of graphical assets that could be used much more simply in code using special schemes for accessing app
...
sheme-handlers.  The object that dynamically generates the site's root is placed there using the simple assignment: {\tt dynamic:/  := MPWMainPage \dots}.
