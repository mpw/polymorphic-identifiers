...
serious drawbacks, and so the fourth option is usually chosen:  simply live with non-uniform
access for custom stores.
...
In addition, the problems presented apply to any custom store, not just dictionaries,
because any custom store has to identify the storage locations (\emph{L-Values}) to reference,
so it has to translate from messages to \emph{L-Values} and back somehow.  These problems
are particularly pervasive in persistence mechanisms 
...
access, where the exact attribute is passed as a parameter at run time.  In fact, the 
difficulties with custom stores 
...
keyed read  & {\tt [person objectForKey:@"name"]; }	 & {\tt [person objectForKey:anAttribute];  }  \\\hline 
keyed write  & {\tt [person setObject:newName forKey:@"name"]; }	 & {\tt [person setObject:newName forKey:anAttribute]; }		 \\\hline 
\end{tabular}
...
\begin{enumerate}
\item Use dictionaries exclusively when parametrized access is required
...
direct keyed access method of Option 2.  So again, although we do have options for indirect
access, none of the options are particularly good as is.
...
\mpw{what about the perf problem with keyed access}
