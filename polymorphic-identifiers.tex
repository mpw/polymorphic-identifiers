...
referring to resources used by the program using human readable names, be they symbolic addresses for
...
Part of the reason may well be familiarity and the weight of existing, string-based APIs such as the POSIX
file functions, but
...
With non-code resources becoming a significant part of modern program~\cite{android-resources}~\cite{ios-resources}, 
and with the World Wide Web pushing us towards greater resource-orientation~\cite{fielding-rest}, we looked at ways of 
interacting more naturally and directly with resources of different kinds.  
...
\subsection{Strings as identifiers and references}
...
small IDE that uses HTTP to upload code and download debugging information from running processes.  Figure~\ref{pi-inuse}
has three IDE windows showing, respectively, the setup code for the web-site, part of the IDE code itself that uploads new method definitions
and finally a window with a live connection to the language learning game running in the iOS simulator in the background (indicated by
the grayed out URL).


\begin{figure}[htbp]
\centering
\includegraphics[scale=0.27,page=1]{PolymorphicIdentifiersInUse.png}
