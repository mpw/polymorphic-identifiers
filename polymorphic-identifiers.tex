...
Like most programming languages, our dialect of Smalltalk\cite{Goldberg1983}, called
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.52,page=1]{ui-widget-generic-access-messages.pdf}
\caption{Widget with parametrized access to DeepThought's {\tt theAnswer} attribute using messages}
...
base performSelector:self setter 
	  withObject:self getContents.
self setContent:(base performSelector:self getter)
\end{lstlisting}
\end{figure}
...
Since the setter and getter messages are related, if only by convention, one way of
reducing the duplication seen in the previous section is using a string to specify
the attribute in question and then derive the getter and setter methods dynamically,
an approach used by Cocoa's {\em Key Value Coding} (KVC) protocol.

KVC adds two methods to every object, {\tt valueForKey:} and {\tt setValue:forKey:},
with the key parameter being the string that's used as an identifier.
...
\footnote{Other Smalltalks use {\tt basicAt:}, but {\tt valueForKey:} will use
...
\mpw{issue: usually have relative paths, but because strings are dumb, we 
	can't express that, so have to perform string processing at call-site
	or duplicate code some more, start with RFC, but images are worse?}

\subsection{Reified identifiers}


Alternatively, we can encapsulate these three
attributes in a separate reference object, such as a {\em ValueHolder}, but this 
object must also be initialized with those parameters. 


\mpw{also need ValueHolder here}

\mpw{more on URLs, matrix of  type  x store, code duplication either in convenience methods 
	or call sites, UIImage etc., that's why we need polymorphism!}
...
\mpw{File object references:  metadata, children}

\mpw{Generally:  pass references, I don't like the example that I currently have}

\mpw{Messaging proxied via the refs.}

One of the problems we identified in section~\ref{identifiers} was the difficulty of 

As we saw in section~\ref{polymorphic-identifiers} and figure~\ref{scheme-eval}, {\em references}
are usually used implicitly to access values and therefore hidden inside the PI system.  However,
it is sometimes necessary to refer to the location that the identifier points to, instead of the 
value.  We have already seen this with the {\em relative} scheme and the {\tt scurl} script, both
of which need to do processing on the reference before creating a value. 
...
access.   The example we saw in listing~\ref{parametrized-reference} using string keys
can now be expressed succinctly and efficiently using a reference (listing~\ref{ref-param}).


\begin{figure}[htbp]
\begin{lstlisting}[style=numbers,label=ref-param,caption=Reference parameters.]
answerReference := ref:deepThought/theAnswer.
answerReference := ref:dictionary/theAnswer.
\end{lstlisting}
\end{figure}
...
error-checking and progress monitoring.   References can be passed to user interface
elements for generic manipulation of domain objects, for example a text field is 
passed a reference to the {\tt name} attribute of a {\tt Person} object.
