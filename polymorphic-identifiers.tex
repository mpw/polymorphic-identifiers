...
Instead, we propose {\em polymorphic identifiers}, a mediated, flexible, encapsulated
and user-extensible  mechanism for resource access to complement messaging
for actions.
...
Our starting point is a programming model similar to that of Smalltalk, where we have objects
...
In Section~\ref{polymorphic-identifiers} we present \emph{polymorphic identifiers} and illustrate their implementation in Section~\ref{implementation}.
...
Identifiers that point to resources come in many different forms ranging from local,
...
Like most programming language, our dialect of Smalltalk allows you to just write 
an identifier that refers to certain kinds of variable in order to retrieve the 

\begin{figure}[htbp]
\begin{lstlisting}[style=L,label=variable-lookup,caption=Interacting with a local variable.]
  theAnswer.
  theAnswer := 42.
\end{lstlisting}
\end{figure}
...
\begin{figure}[htbp]
\begin{lstlisting}[style=L,label=variable-lookup,caption=Reified local variable access.]
  (localContext slotAtName:'theAnswer') GET.
  (localContext slotAtName:'theAnswer') PUT:42.
\end{lstlisting}
\end{figure}
...
\begin{figure}[htbp]
\begin{lstlisting}[style=L,label=many-urls,caption=Looking up RFCs]
