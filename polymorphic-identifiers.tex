...
parametrized identifiers (section~\ref{parametrized}), and finally some implementation details (section~\ref{implementation}).
In section~\ref{evaluation} we evaluate the system and look at related work in section~\ref{related-work}.
...
hypothetical \emph{ref(identifier)} function and then the reference is used to look up an actual
...
\emph{direct reference}, where identifiers directly resolve to values, \emph{message-based} lookup
used for encapsulation,
\emph{string-based} lookup often used with extensible data structures or filesystem access and
...
\emph{Objective-Smalltalk} allows you to just write 
...
a file or an environment variable exists (see listing~\ref{isBound}).
...
to a file (see figure~\ref{symbolic-link}).
...
boost to lookup operations compared to evaluating strings at runtime.  Table~\ref{var-speed}
and figure~\ref{var-speed-graph} 
compare evaluating the identifier {\tt var:stream/target/target} with using the Objective-C 
...
\end{tabular}
\caption{Time for  {\tt var:} scheme vs. \emph{Key Value Coding} for two-element path access}
\label{var-speed}
\end{center}
\end{table}
...
Table~\ref{http-server-speed} shows the performance, in requests/s of several different kinds of (composed)
schemes, with the apache web server as comparison.  

\begin{table}
\begin{tabular}{|l|c|c|r|r|r|r|r|r|r|r|r|} \hline
...
\begin{figure}
\includegraphics[scale=0.5,page=2]{server-perf.pdf}
\caption{Performance of different schemes bridged to http vs. apache}
\label{http-server-speed-graph}
\end{figure}
