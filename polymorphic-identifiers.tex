...
access in lieu of plain identifiers.  Self~\cite{UngarS87} and Newspeak~\cite{newspeak} take this
...
Although scheme-handlers can be exported via http, Polymorphic Identifiers leave the decision of wether to present a messaging
...
E even allows custom schemes to be defined, but for only for read-access, separate from other identifiers and without the ability to extract references.
...
built-in rules for wether they are evaluated or treated as references, rather than having explicit control as with Polymorphic Identifiers.

A different approach to resource access comes from the operating system community:  Plan9\cite{plan9names}\cite{plan9network} 
integrates a wide variety of local and remote resources and services into a single directory tree that is made available on a per-process basis.
User level filesystems like FUSE\cite{fuse} or the BSD Pass-to-User-Space\cite{kantee:puffs} 
system bring some of these ideas to commercial operating systems, but must have access mediated via the kernel and visible to all
...
scheme-handlers, but don't require communication to be string based or mediated by the operating system kernel, although scheme-handlers
can be exported as filesystems using FUSE if desired.  In addition
Polymorphic Identifiers integrate such resource access with a general purpose programming language without the need for  
string-based identifiers and I/O system calls, rather than requiring domain-specific languages such as Unix shells for direct access.
...
Smalltalk reflective facilities \cite{reflective-st} can be used to build custom messaging facilities similar to the
way messaging is handled by sending HOMs to references in Polymorphic Identifiers, and in fact, HOM
can be implemented using those reflective facilities.  


 have long been used to build
